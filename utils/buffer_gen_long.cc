    ...
    while(ievt<=NEVENTS && !eof) {
      if(getline(*datafile, line)) {
        if(line.find("Event") != string::npos) {
          ...
          // Write two buffer words per data
          rxdata0.push_back(indata >> 48);
          rxdata0.push_back(0);
          rxdata0.push_back(0);
          rxdata0.push_back(0x31000 | addr);

          rxdata0.push_back(indata & 0xFFFF);
          rxdata0.push_back((indata >> 16) & 0xFFFF);
          rxdata0.push_back((indata >> 32) & 0xFFFF);
        ...
        // update header word with number of data
        if(ievt==1) {
          rxdata0[ihead] = (rxdata0[ihead] | (ndata << 8));
        } else if(ievt==2) {
          rxdata0[ihead] = (rxdata0[ihead] | ndata);
        }
      }
    }
  }
